...
export default function TableOfContents ({ blockMap, frontMatter, pageTitle, showScrollElement }) {
  ...
  let page
  if (pageTitle) {
    const firstBlockId = Object.keys(blockMap.block ?? {})[0]
    page = getBlockValueById(firstBlockId)
  } else {
    const collectionIds = new Set(
      Object.keys(blockMap.collection ?? {}).map(normalizeId)
    )

    page = blockValues.find(block =>
      block?.parent_id &&
      collectionIds.has(normalizeId(block.parent_id))
    )
  }

  // Fallback to the first page-like block when parent linkage differs.
  if (!page) {
    page =
